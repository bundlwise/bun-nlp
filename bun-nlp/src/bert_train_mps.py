...
# Import TorchCRF for sequence modeling
try:
...
class MultiTaskConfig:
    ...
    classification_weight: float = 4.0  # Increased from 1.0 to 4.0 for company classification
    ner_weight: float = 3.5  # Increased from 2.5 to 3.5 to improve entity extraction
    ...
    use_class_weights: bool = False  # Enable to use class weights for imbalanced company data
    
    # NER improvements
    use_crf: bool = False  # Enable CRF for improved NER sequence modeling
...
class MultiTaskBertModel(BertPreTrainedModel):
    ...
    def __init__(self, config: MultiTaskBertConfig):
        ...
        # Company classification head (which company sent this email) - Improved architecture
        self.classification_head = nn.Sequential(
            nn.Linear(config.hidden_size, config.hidden_size),
            nn.LayerNorm(config.hidden_size),
            nn.GELU(),
            nn.Dropout(config.classifier_dropout * 0.8),  # Reduced dropout for better learning
            nn.Linear(config.hidden_size, config.hidden_size // 2),
            nn.LayerNorm(config.hidden_size // 2),
            nn.GELU(),
            nn.Dropout(config.classifier_dropout * 0.8),
        ...
        self.ner_head = nn.Sequential(
            ...
            nn.LayerNorm(config.hidden_size),
            nn.GELU(),
            nn.Dropout(config.classifier_dropout * 0.7),  # Reduced dropout for better learning
            nn.Linear(config.hidden_size, config.hidden_size),
            nn.LayerNorm(config.hidden_size),
            nn.GELU(),
            nn.Dropout(config.classifier_dropout * 0.7),
            nn.Linear(config.hidden_size, config.hidden_size // 2),
            nn.LayerNorm(config.hidden_size // 2),
            nn.GELU(),
            nn.Dropout(config.classifier_dropout * 0.7),
        ...
        # Add CRF layer for improved NER sequence modeling
        self.use_crf = hasattr(config, 'use_crf') and config.use_crf and CRF_AVAILABLE
        if self.use_crf and CRF_AVAILABLE:
            self.crf = CRF(config.num_ner_labels, batch_first=True)
    ...
    def forward(
        ...
        if classification_labels is not None:
            if class_weights is not None:
                # Use class weights for imbalanced company data
        ...
        if ner_labels is not None:
            ...
            # Calculate entity type weights based on frequency
            if hasattr(self.config, 'ner_label_weights') and self.config.ner_label_weights is not None:
                # Use pre-calculated weights if available
                ner_weights = self.config.ner_label_weights.to(ner_logits.device)
                loss_fct = nn.CrossEntropyLoss(weight=ner_weights, ignore_index=-100)
            else:
                # Default to standard loss
                loss_fct = nn.CrossEntropyLoss(ignore_index=-100)
            
            # Use CRF if available
            if self.use_crf and CRF_AVAILABLE:
                # Create mask for CRF
                crf_mask = attention_mask.bool()
                
                # Convert labels to proper format for CRF
                # Replace -100 with 0 (will be masked anyway)
                ner_labels_crf = ner_labels.clone()
                ner_labels_crf[ner_labels_crf == -100] = 0
                
                # Compute negative log likelihood loss
                ner_loss = -self.crf(ner_logits, ner_labels_crf, mask=crf_mask, reduction='mean')
            else:
...
class MultiTaskTrainer:
    ...
    def __init__(self, config: MultiTaskConfig):
        self.config = config
        self.device = torch.device("mps") if torch.backends.mps.is_available() else self._setup_device()
    ...
    def load_and_process_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Calculate NER label weights based on frequency (inverse frequency weighting)
            if entity_counts:
                total_entities = sum(entity_counts.values())
                ner_label_weights = torch.ones(self.config.num_ner_labels)
                # Set weight for 'O' tag (non-entity)
                ner_label_weights[0] = 0.5  # Lower weight for non-entity tokens
                
                # Set weights for entity types based on inverse frequency
                for entity_type, count in entity_counts.items():
                    if entity_type in ner_label_map:
                        idx = ner_label_map[entity_type]
                        # Inverse frequency with smoothing
                        weight = min(5.0, total_entities / (count + 10))  # Cap at 5.0 to prevent extreme values
                        ner_label_weights[idx] = weight
                
                # Normalize weights
                ner_label_weights = ner_label_weights / ner_label_weights.mean()
                
                # Log the weights for top entities
                sorted_weights = [(entity_type, ner_label_weights[ner_label_map[entity_type]].item()) 
                                 for entity_type in sorted(entity_counts.keys(), 
                                                         key=lambda x: entity_counts[x])]
                logger.info(f"NER label weights for top 10 rarest entities: {sorted_weights[:10]}")
                
                # Store in config
                self.config.ner_label_weights = ner_label_weights
            
            # Compute class weights for imbalanced company data if enabled
            class_weights = None
            if self.config.use_class_weights:
                # Count examples per company
                company_counts = {}
                for example in data:
                    company = example.get('category', 'unknown')
                    company_counts[company] = company_counts.get(company, 0) + 1
                
                # Compute class weights (inverse frequency)
                class_labels = [classification_label_map[example['category']] for example in data if 'category' in example]
                ...
                logger.info(f"Using class weights for {len(class_weights)} company classes")
                
                # Log class weights for top 5 most imbalanced classes
                sorted_weights = sorted([(categories[i], w.item()) for i, w in enumerate(class_weights)], 
                                       key=lambda x: x[1], reverse=True)
                logger.info(f"Top 5 class weights: {sorted_weights[:5]}")
    ...
    def _train_single_fold(self, processed_data: Dict[str, Any]):
        ...
        # Get class weights for imbalanced company data
        class_weights = processed_data.get('class_weights', None)
        ...
        for epoch in range(self.config.num_train_epochs):
            ...
            for step, batch in enumerate(progress_bar):
                ...
                if self.config.fp16 and self.scaler is not None:
                    with autocast():
                        outputs = model(**batch, class_weights=class_weights)
                        loss = outputs['loss']
    ...
    def _train_cross_validation(self, processed_data: Dict[str, Any]) -> Dict[str, float]:
        """Train with k-fold cross-validation"""
        logger.info(f"Starting {self.config.cv_folds}-fold cross-validation")
        
        # Extract data and CV splits
        data = processed_data['data']
        cv_splits = processed_data['cv_splits']
        classification_label_map = processed_data['classification_label_map']
        ner_label_map = processed_data['ner_label_map']
        sentiment_label_map = processed_data['sentiment_label_map']
        subscription_label_map = processed_data['subscription_label_map']
        ...
        for fold, (train_idx, val_idx) in enumerate(cv_splits):
            ...
            fold_output_dir = Path(self.config.output_dir) / f"fold_{fold + 1}"
            ...
            # Further split train data to get a test set
            train_data, test_data = train_test_split(
                train_data, test_size=0.1, random_state=self.config.seed + fold,
                stratify=[example['category'] for example in train_data]
            )
            ...
            # Temporary config override for this fold
            original_output_dir = self.config.output_dir
            self.config.output_dir = str(fold_output_dir)
            ...
            for epoch in range(self.config.num_train_epochs):
                logger.info(f"Fold {fold + 1}, Epoch {epoch + 1}/{self.config.num_train_epochs}")
                
                # Training phase
                model.train()
                total_loss = 0.0
                
                progress_bar = tqdm(train_dataloader, desc=f"Training Fold {fold + 1}, Epoch {epoch + 1}")
                ...
                for step, batch in enumerate(progress_bar):
                    ...
                    if global_step % self.config.eval_steps == 0:
                        ...
                        if is_best:
                            best_eval_score = current_eval_score
                            logger.info(f"Fold {fold + 1}: New best {self.config.metric_for_best_model}: {best_eval_score:.4f}")
                        ...
                        # Early stopping check
                        if early_stopping(current_eval_score, model):
                            logger.info(f"Fold {fold + 1}: Early stopping triggered at step {global_step}")
                            if self.config.load_best_model_at_end:
                                early_stopping.restore_best_model(model)
                            break
                ...
                logger.info(f"Fold {fold + 1}: Running final evaluation...")
                fold_metrics = self.evaluate(model, test_dataloader, processed_data, prefix=f"fold_{fold + 1}")
                
                # Save fold model
                fold_model_dir = Path(self.config.output_dir) / "final_model"
                if hasattr(model, 'module'):
                    model.module.save_pretrained(fold_model_dir)
                else:
                ...
                # Log fold results
                logger.info(f"Fold {fold + 1} completed with metrics: {fold_metrics}")
        ...
        # Create ensemble model (optional)
        # Here we could implement model ensembling from the different folds
...
def main():
    ...
    parser.add_argument("--use_class_weights", action="store_true",
                        help="Use class weights for imbalanced company data")
    parser.add_argument("--use_crf", action="store_true",
                        help="Use CRF layer for improved NER sequence modeling")
    
    # Multi-task weights
    parser.add_argument("--classification_weight", type=float, default=4.0,
                        help="Weight for classification loss")
    parser.add_argument("--ner_weight", type=float, default=3.5,
